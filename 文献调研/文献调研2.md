...
# Towards $S^2$-Challenges Underlying LLM-Based Augmentation for Personalized News Recommendation

## 主要的问题
**结构缺陷**：目前基于 LLM 的数据增强主要是在“语义层面”进行的 。在增强过程中，LLM 根据每个用户的历史点击记录，独立地为该用户生成“虚拟”新闻。导致增强后的“用户-新闻”二部图在结构上表现为多个**相互隔离的孤岛**，协同过滤信号无法在不同用户之间传播，从而限制了推荐性能。
**语义噪声**：llm利用通用知识来扩展原本处于“封闭系统”内的推荐模型时，不可避免地会引入与用户实际阅读偏好无关的信息（如无关的实体或背景知识）

![[Pasted image 20260129000140.png]]
左：结构感知优化
右：语义感知去噪


# 《Multi-Interest Matching for Personalized News Recommendation with Large Language Models》(MIPNR)

## 主要问题

传统推荐系统过度依赖点击信号而忽视非点击信息，打破单一的“点击即兴趣”假设，通过 LLM 挖掘非点击新闻中的潜在兴趣。

## 主要工作
![[Pasted image 20260129124736.png]]
![[Pasted image 20260129124808.png]]
![[Pasted image 20260129124915.png]]
# LLM4Rerank: LLM-based Auto-Reranking Framework for Recommendations
## 主要问题
摆脱传统重排序中复杂的“权重超参数搜索”，实现一种能够理解业务逻辑并自动调整排序权重的智能化方案
## 主要工作
![[Pasted image 20260129141627.png]]
## 问题
这感觉对LLM消耗非常大，耗时也很长，此外遗忘问题应该也会比较严重

# CoRA: Collaborative Information Perception by Large Language Model’s Weights for Recommendation

## 主要问题
1. LLM 在预训练阶段学到了海量的**通用知识**。在微调阶段强制它学习特定领域的推荐知识（协同过滤信号）时，这两种知识会发生冲突。模型可能为了记住“用户 A 喜欢物品 B”这种协同关系，而丧失了对新闻标题细微语义的理解能力，导致模型变笨
2.  协同信息（如 User ID 的 Embedding）通常处于一个结构化的向量空，而文本信息处于 NLP 的语义空间。两个空间的分布完全不同，LLM 的注意力机制很难同时处理这种“非自然语言”的信号，导致协同信息无法被有效理解
## 主要工作
![[Pasted image 20260129143435.png]]
1. Encoding部分用LightGCN学习用户和物品的编码
2. ![[Pasted image 20260129144103.png]]表示
出权重变化。再融合到LLM的内部的一个线形层权重中![[Pasted image 20260129144305.png]]
3. 训练过程：（1）预训练部分：给定物品的协同向量 $e_i$，让 LLM 输出该物品的标题或类别描述。（2）微调部分：应用于推荐任务，输出是否进行推荐。
LLM 的 Backbone完全冻结，只训练新增加的LCP模块中的部分。
...
隐私遗忘
## 方法
![[Pasted image 20260129151550.png]]
